...
# Implementar as funções para cálculos de potenciação, radiciação, trigonometria e logarítmos
...
def logaritmo(numero, base):
    """Retorna o logarítmo de um número na base especificada."""
...
try:
    ...
    num2_str = input("Digite o segundo número (ou raiz/expoente/base para as outras operações.)")
    ...
    if not num1_str.replace('.','',1).isdigit() or not num2_str.replace('.','',1).isdigit():
        ...
        if operador in ['+','-','*','/','**','rad','sen','cos','tan','log']:
            # Passo 4: integrar as novas funções ao programa.
            if operador == '+':
                ...
                resultado = seno(num1) # assume que o primeiro número é o ãngulo em graus.
            elif operador == 'cos':
                resultado = cosseno(num1) # assume que o primeiro número é o ãngulo em graus.
            elif operador == 'tan':
                resultado = tangente(num1) # assume que o primeiro número é o ãngulo em graus.    
            elif operador == 'log':
            ...
            raise ValueError("Operador inválido. Use: +, -, *, /, **, rad, sen, cos, tang, log.")
